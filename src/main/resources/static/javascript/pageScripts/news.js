...
function createNewsElement(index){
    ...
    let testImage = "null"; //"https://o.aolcdn.com/images/dims?resize=1200%2C630&crop=1200%2C630%2C0%2C0&quality=95&image_uri=https%3A%2F%2Fs.yimg.com%2Fos%2Fcreatr-uploaded-images%2F2020-11%2Fa94688a0-2b42-11eb-97de-40ca186f0af4&client=amp-blogside-v2&signature=69f5720b09326b768c479c84153fa7879d072922";
    ...
    let articleHTML = $("" +
        "<li ='removeElementIfNull("+testImage+")' class='list-group-item row bg-transparent my-2'>" +
            "<div class='col'>" +
                "<div id='Title' class='row'><a href='"+testArticleLink+"' target='_blank' rel='noopener noreferrer'><h2>"+testTitle+"</h2></a></div>" +
                "<div class='row'>" +
                imageElement +
                    "<div class='col-8'>" +
                ...
                "<footer class='row text-muted'>" +
                    "<span class='col'>"+testSourceName+"</span>" +
                    "<span class='col'>"+testAuthor+"</span>" +
                    "<span class='col'>"+$.format.date(testDate, 'dd-MM-yyyy')+"</span>" +
                "</footer>" +
            "</div>" +
        "</li>"
    )
...
function removeElementIfNull(testImage){
    console.log("what is it?" + testImage);
    if(testImage === "null"){
        $("#imageParent").remove();
    }
}
